...
let load_ps file bbox_opt options =
  if not Config.lib_ps then failwith "ps is not supported" else
  let resx, resy =
  ...
  let command =
    ...
    | None ->
      Printf.sprintf
        ...
        Config.path_gs resx resy tmpfile file
    | Some (x1, y1, x2, y2) ->
      ...
      Printf.sprintf
        ...
        Config.path_gs resx resy width height tmpfile (-x1) (-y1) file in
